...
class PostList(ListView):
    model = Post
    ordering = '-pk'          #-pk 하면 역순으로 정렬됨
    paginate_by = 10            #한페이지에 10개씩 보여주겠다는 것 
...
class PostUpdate(LoginRequiredMixin, UpdateView):
    ...
    template_name = 'free_board/post_update_form.html'
...
class PostSearch(PostList):
    paginate_by = None                      #서치 할게 많으면 paginate 하는 것

    def get_queryset(self):
        q = self.kwargs['q']
        post_list = Post.objects.filter(
            Q(title__contains=q) | Q(tags__name__contains=q)        # 파이프는 or, 언더바 두개 쓰는건 장고에서 쿼리 표현식 약속
        ).distinct()             #중복을 방지 하는것 distinct 
        return post_list
